  ...
  <div class="home">
    ...
    <div v-for="item in homelist" :key="item.id">
      <div class="activity-panel" v-if="item.type==1">
        <el-row>
          <el-col class="content" :span="8" v-for="(o,i) in item.panelContents" :key="i">
      ...
      <section class="w mt30 clearfix" v-if="item.type==2">
        <mshelf :title="item.name">
          <div slot="content" class="hot">
            <goods v-for="(o,i) in item.panelContents" :key="i" :goods="o"></goods>
          </div>
        </mshelf>
      </section>
      <section class="w mt30 clearfix" v-if="item.type==3">
        ...
        <mshelf :title="item.name">
          <div slot="content" class="floors">
            <div v-for="(o,i) in item.panelContents" :key="i" v-if="o.type!==0" class="imgbanner">
              <img :src="o.picUrl" alt />
            </div>
            <goods :goods="o" v-for="(o,i) in item.panelContents" :key="i" v-if="o.type==0"></goods>
          </div>
        </mshelf>
      </section>
    </div>
...
export default {
  ...
  async created() {
    try {
      let res = await this.$http.get("api/goods/home");
      let data = res.data;
      this.homelist = data.result;
      if (res.status == 200) {
        let item = data.result.find((item) => {
          return item.type == 0;
        });
